...
void ucsc_hn_lib::RenaDataWriter::writeFile ( uint8_t channel, std::shared_ptr<rogue::interfaces::stream::Frame> frame) {
   ...
   if ( fd_ >= 0 ) {
      ...
      fromFrame(src,channel,1);
      fromFrame(src,fpgaId,1);
      fromFrame(src,renaId,1);
      fromFrame(src,nodeId,1);
      fromFrame(src,polarity,1);
      fromFrame(src,timeStamp,8);
      fromFrame(src,frameId,4);
      fromFrame(src,phaData,2);
      fromFrame(src,uData,2);
      fromFrame(src,vData,2);

      sprintf(buffer, "%i %i %i %i %i %i %i %i %li\n",nodeId,fpgaId,renaId,channel,polarity,phaData,uData,vData,timeStamp);
