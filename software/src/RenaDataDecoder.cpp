...
void ucsc_hn_lib::RenaDataDecoder::setup_python() {
   bp::class_<ucsc_hn_lib::RenaDataDecoder, ucusc_hn_lib::RenaDataDecoderPtr, bp::bases<ris::Master,ris::Slave>, boost::noncopyable >("RenaDataDecoder",bp::init<uint8_t>());
...
void ucsc_hn_lib::RenaDataDecoder::getRxFrameCount() {
   return rxFrameCount_;
}

void ucsc_hn_lib::RenaDataDecoder::getRxSampleCount() {
   return rxSampleCount_;
}

void ucsc_hn_lib::RenaDataDecoder::getRxDropCount() {
   return rxDropCount_;
}

void ucsc_hn_lib::RenaDataDecoder::acceptFrame ( ris::FramePtr frame ) {
   ris::FramePtr newFrame;
   ...
   // Computed CRC
   expCrc = 0;
   for (x=0; x < frame->getPayload()-3) expCrc = crc8_table_[crc ^ srcData[x]];
   ...
   for ( x=0; x < 36; x++ ) {
      ...
      if ( readPHA or readUV ) {
         ...
         nFrame = requestFrame(23,true);
         nFrame.setPayload(23);
         nFrame.setDestination(2);
         dst = nFrame->begin();
         ...
         // Start frame data
         toFrame(dst,i,1); // Channel ID
         toFrame(dst,fpgaId,1);
         toFrame(dst,renaId,1);
         toFrame(dst,nodeId_,1);
         toFrame(dst,polarity,1);
         toFrame(dst,timeStamp,8);
         toFrame(dst,rxFrameCount_,4);
         toFrame(dst,phaData,2);
         toFrame(dst,uData,2);
         toFrame(dst,vData,2);
